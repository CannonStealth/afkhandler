...
class AFKHandler extends discord_js_1.Client {
    ...
    async Commands(dir, options) {
        ...
        this._loader(dir, (command, file) => {
            if (!command.name)
                throw new Error("AFKHandler commands " +
                    file +
    ...
    convert(number, decimals = 0) {
        ...
        return this._convertLong([days, hours, minutes, seconds].map(t => t.toString()));
    }
    _convertLong(converted) {
        {

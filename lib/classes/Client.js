...
class AFKHandler extends discord_js_1.Client {
    ...
    async SlashCommands(dir, callback) {
        ...
        this._loader(dir, (file, fileName) => {
            if (!file.name)
                throw new Error("AFKHandler commands " + fileName + " ERROR: You didn't put any name on a command");
            if (!file.callback &&
                !file.run &&
                !file.emit &&
                !file.execute &&
                !file.fire)
                throw new Error("AFKHandler commands " + fileName + " ERROR: You didn't put any run function on a command");
            if (file.cooldown &&
                typeof file.cooldown === "string" &&
                !/^(\d+[mhsd]\s?)+$/gi.test(file.cooldown))
                throw new Error("AFKHandler commands " + fileName + " ERROR: Invalid time to convert");
    ...
    Events(dir, callback) {
        this._loader(dir, (event, file) => {
            if (!event.name)
                throw new Error("AFKHandler events " + file + " ERROR: there's no event name");
            const fun = event.callback || event.run || event.execute || event.fire || event.emit;
            if (!fun)
                throw new Error("AFKHandler events " + file + " ERROR: there's no run function");
            if (!event.once)
                this.on(event.name, fun.bind(null, { client: this, gadget: this.gadget }));
            else
                this.once(event.name, fun.bind(null, { client: this, gadget: this.gadget }));
            if (callback)
                callback(event, file);
        });
        return this;
    }
    Features(dir, callback) {
        this._loader(dir, (feature, file) => {
            const run = feature.run || feature.execute || feature.callback || feature.emit || feature.fire;
            if (!run)
                throw new Error("AFKHandler features " + file + " ERROR: there's no run function");
            run({ client: this, gadget: this.gadget });
            if (callback)
                callback(feature, file);
        });
        return this;
    }

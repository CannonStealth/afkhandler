...
class AFKHandler extends discord_js_1.Client {
    ...
    async _loader(dir, callback) {
        const files = await promises_1.readdir(path_1.join(__dirname, dir));
        for (const file of files) {
            const stat = await promises_1.lstat(path_1.join(__dirname, dir, file));
            if (stat.isDirectory())
                this._loader(path_1.join(__dirname, dir));
            else if (!file.endsWith(".js") &&
                !file.endsWith(".coffee") &&
                (!file.endsWith(".ts") || file.endsWith(".d.ts")))
                continue;
            const imported = (await Promise.resolve().then(() => __importStar(require(path_1.join(__dirname, dir, file))))).default;
            console.log(file);
            if (callback)
                callback(imported);
        }
